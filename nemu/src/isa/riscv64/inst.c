...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, printf("inst %lx, dest =%lx\n",s->dnpc,dest);int64_t a = src1; int64_t b = src2;s->dnpc = ((int)a >= (int)b) ?( s->pc + dest):( s->snpc);printf("inst %lx\n",s->dnpc); printf("src11 = 0x%ld, src2 = 01x%ld \n",src1,src2));

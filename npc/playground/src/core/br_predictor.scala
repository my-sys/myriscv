...
class Br_predictor extends Module{
	...
	val BTB = Mem(64,UInt(128.W))
	//val PHT = Mem(64,UInt(2.W))
	val RAS = Mem(6,UInt(64.W))
	...
	when(io.br_info.valid){
		PHT(update_index) := Mux(update_taken,Mux(update_pht_data === "b11".U,update_pht_data,update_pht_data + 1.U),
		Mux(update_pht_data === "b00".U,update_pht_data,update_pht_data - 1.U))
		when(mispredict){
	...
	val pre_next_pc = Wire(UInt(64.W))

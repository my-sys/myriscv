...
class IBuf extends Module{
	...
	val ibuf_inst_read = ibuf_inst(reg_tail)
	val ibuf_is_pre_read = ibuf_is_pre(reg_tail)
	val ibuf_valid_read = ibuf_valid(reg_tail)
	when(flush){
		reg_head	:= 0.U 
		reg_tail 	:= 0.U 
		reg_ibuf_size := 0.U
...
//----------------------Pre Info--------------------------------
	...
	io.get_pre_info.pre_next_pc := Mux(io.get_pre_info.valid,get_pre_next_pc,0.U)

// Verilated -*- C++ -*-
// DESCRIPTION: Verilator output: Design internal header
// See Vriscv_soc.h for the primary calling header

#ifndef VERILATED_VRISCV_SOC___024UNIT_H_
#define VERILATED_VRISCV_SOC___024UNIT_H_  // guard

#include "verilated.h"

class Vriscv_soc__Syms;

class Vriscv_soc___024unit final : public VerilatedModule {
  public:

    // INTERNAL VARIABLES
    Vriscv_soc__Syms* const vlSymsp;

    // CONSTRUCTORS
    Vriscv_soc___024unit(Vriscv_soc__Syms* symsp, const char* v__name);
    ~Vriscv_soc___024unit();
    VL_UNCOPYABLE(Vriscv_soc___024unit);

    // INTERNAL METHODS
    void __Vconfigure(bool first);
} VL_ATTR_ALIGNED(VL_CACHE_LINE_BYTES);


#endif  // guard
